...
export function CreateProjectModal({ open, onClose, teamId }) {
  const { db, createProject, createProjectFromTemplate, hasPermission } = useApp();
  const [mode, setMode] = useState('blank');
  const [form, setForm] = useState({ title: '', desc: '', horizon: '', start: today(), due: today(), status: 'new', templateId: '' });
  ...
  function submit() {
    ...
    if (project) {
      setForm({ title: '', desc: '', horizon: '', start: today(), due: today(), status: 'new', templateId: '' });
      setMode('blank');
      onClose();
    }
  }
